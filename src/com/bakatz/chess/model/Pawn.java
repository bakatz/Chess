    ...
    extends ChessGamePiece
{
    ...
    protected ArrayList<ChessLocation> calculatePossibleMoves( ChessGameBoard board )
    {
        ...
        if ( isPieceOnScreen() )
        {
            ...
            while ( count <= maxIter )
            { // only loop while we have open slots and have not passed our
              // limit
                if ( isOnScreen( currRow, pieceColumn )
                    && board.getCell( currRow,
                        pieceColumn ).getPieceOnSquare() == null )
                {
                    moves.add( new ChessLocation(currRow , pieceColumn) );
                }
                else
                {
                    break;
            ...
            if ( getColorOfPiece() == ChessGamePiece.WHITE )
            {
                ...
                {
                    moves.add( new ChessLocation( pieceRow - 1 , pieceColumn - 1 ) );
                }
                if ( isEnemy( board, pieceRow - 1, pieceColumn + 1 ) )
                {
                    moves.add( new ChessLocation( pieceRow - 1 , pieceColumn + 1 ) );
